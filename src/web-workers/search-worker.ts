...
self.onmessage = async (event: MessageEvent<WorkerMessage>) => {
    ...
    try {
        ...
        switch (type) {
            ...
            case 'EXPORT_INDEX':
                initialize();
                result = await searchIndex.exportIndex();
                sendResponse('EXPORT_RESULT', result, requestId);
                break;

            case 'IMPORT_INDEX':
                initialize();
                await searchIndex.importIndex(payload.data);
                result = { success: true };
                sendResponse('IMPORT_COMPLETE', result, requestId);
                break;
